...
function CheckTranslation(translation, expectedTagsData, callback) {
   ...
   */
  this._origOnCloseTag = this._parser.onclosetag.bind(this._parser);
  this._parser.onclosetag = this._replacementOnCloseTag.bind(this);
...
CheckTranslation.prototype = {
  ...
  _onOpenTag: function(tagName, tagAttributes) {
    ...
    try {
      ...
      if (selfClosingElements.has(tagName)) {
        // The parser does not call _replacementOnCloseTag for these
        // elements, and without the call, the element remains on
        // the openElementStack and causes an error. See #7.
        this._checkIfMismatchedOpenAndCloseTags(tagName);
      }
    } catch (err) {
      this._finish(err);
    }
  ...
  _checkIfMismatchedOpenAndCloseTags(tagName) {
    if (this._done) {
      return;
    }
  ...
  _replacementOnCloseTag(tagName, ...args) {
    this._origOnCloseTag(tagName, ...args);
    return this._checkIfMismatchedOpenAndCloseTags(tagName);
  },
};
